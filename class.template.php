		...
		public function meta() {
				...
				if ( is_array( $this->page_meta ) && sizeof( $this->page_meta ) > 0 ) {
						foreach ( $this->page_meta as $key => $value ){   
                                                    ...
                                                    if( $key == 'description' ){$descriptionFound = true;}
                                                }
                                }
			...
			if ((isset($_GET['object'])) && ($_GET['module'] == 'content')) {
				...
				<meta property="og:description" content="<?PHP 
                                    //if you found meta description, you insert it also in og:description
                                    if ($descriptionFound){
                                        echo MPut::html_attr( $this->page_meta['description'] ) ; 
                                    }else{
                                        echo mb_substr(str_replace('"', '', strip_tags($ContentDesc->get_text())), 0, 155, 'UTF-8'); 
                                    }
                                ?>" />
				...
				<meta name="twitter:description" content="<?PHP
                                    //if you found meta description, you insert it also in twitter:description
                                    if ($descriptionFound){
                                        echo MPut::html_attr( $this->page_meta['description'] ) ; 
                                    }else{
                                        echo mb_substr(str_replace('"', '', strip_tags($ContentDesc->get_text())), 0, 155, 'UTF-8'); 
