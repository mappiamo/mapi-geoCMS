		...
		function getmetadata($id) {
			...
			if ($content_data) {
				if ($content_data['type'] == 'route') {
					...
					$get_similar_byaddress = ORM::for_table('contents')->select_many('id', 'title', 'type')->where_like('address', '%' . $SeekTitle . '%')->find_array();
				...
				} elseif ($content_data['type'] == 'place') {
					...
					$get_similar_byaddress = ORM::for_table('contents')
												->raw_query('SELECT id, title, type, address, (3959 * acos(cos(radians(:latitude)) * cos(radians(lat)) * cos(radians(lng) - radians(:longitude)) + sin(radians(:latitude))  * sin(radians(lat)))) * 1000 AS distance FROM contents HAVING distance < :radius ORDER BY distance ASC LIMIT 10',
													array("latitude" => $content_data["lat"], "longitude" => $content_data["lng"], "radius" => 2000))->find_array();

				} elseif ($content_data['type'] == 'event') {
					//$get_similar_byid = ORM::for_table('categories')->select_many('id', 'contents', 'title')->where_like('contents', '%{' . $id . '}%')->find_array();
					$get_similar_byaddress = ORM::for_table('contents')
																			->raw_query('SELECT id, title, type, address, (3959 * acos(cos(radians(:latitude)) * cos(radians(lat)) * cos(radians(lng) - radians(:longitude)) + sin(radians(:latitude))  * sin(radians(lat)))) * 1000 AS distance FROM contents WHERE type = \'event\' HAVING distance < :radius ORDER BY distance ASC LIMIT 10',
				...
				if (count($get_similar_byid) > 0) {
					...
					$get_similar_markers = ORM::for_table('contents')->select_many('id', 'title', 'type')->where_raw($Content_where)->find_array();
				} else {
